...
fn main() {
    ...
    // receive quote from platform daemon
    // TODO: receive quote length first
    ...
    if pck_cert_chain.len() > 10 {
        panic!("Certificate chain longer than 10 is not allowed.");
    }

    // grab individual certs from chain
    let pck_leaf_cert = &pck_cert_chain[0];
    let pck_intermed_cert = &pck_cert_chain[1];
    let pck_root_cert = &pck_cert_chain[2];
    println!("PCK cert chain loaded...");

    // verify PCK certificate chain issuers and signatures
    verify_chain_issuers(&pck_root_cert, &pck_intermed_cert, &pck_leaf_cert);
    verify_chain_sigs(
        pck_root_cert.clone(),
        pck_intermed_cert.clone(),
        pck_leaf_cert.clone(),
    );
